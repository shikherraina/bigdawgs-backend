...
export const createUpiPayment = async (req: Request, res: Response) => {
  try {
    ...
    try {
      await sendApprovalEmail(payment, transactionId, upiUrl, amount, merchantName);
    } catch (emailError) {
...
// Email notification function - ONLY ONE COPY
async function sendApprovalEmail(
  ...
  try {
    ...
    // Send email
    const mailOptions = {
      from: process.env.FROM_EMAIL || `"${merchantName}" <noreply@bigdawgs.com>`,
